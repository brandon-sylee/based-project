 ...
 */
@RestController
@RestControllerAdvice(basePackageClasses = NewsRestController.class)
@RequestMapping("api")
public class NewsRestController {
    private final GoogleNewService googleNewService;

    @Autowired
    public NewsRestController(GoogleNewService googleNewService) {
        this.googleNewService = googleNewService;
    }
     ...
     */
    @GetMapping("news")
    public Callable<Message<SyndEntry>> news() throws NewFeedException {
        try {
     ...
     */
    @GetMapping("gnews")
    public DeferredResult<Message<SyndEntry>> gnews() throws NewFeedException {
        try {
            DeferredResult<Message<SyndEntry>> deferredResult = new DeferredResult<>();
            CompletableFuture
