 ...
 */
public class BrandonCssCompressor {
    private StringBuffer srcsb = new StringBuffer();

    public BrandonCssCompressor(Reader in) throws IOException {
        // Read the stream...
        int c;
        while ((c = in.read()) != -1) {
            srcsb.append((char) c);
    ...
    public String compress(int libeBreakPosition) throws IOException {
        Pattern p;
        Matcher m;
        ...
        if (libeBreakPosition >= 0) {
            ...
            while (i < sb.length()) {
                char c = sb.charAt(i++);
                if (c == '}' && i - linestartpos > libeBreakPosition) {
                    sb.insert(i, '\n');
                    linestartpos = i;
                }
            }
